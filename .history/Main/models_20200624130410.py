...
class TripManager(models.Manager):
    def trip_validator(self, post):
        ...
        if len(post['destination']) < 3:
            errors['destination'] = "Job title must be at least 3 characters"
        if len(post['start_date']) < 3:
            errors['start_date'] = "Job location must be at least 3 characters"
        if len(post['end_date']) < 3:
            errors['end_date'] = "Job description must be at least 3 characters"
        if len(post['plan']) < 3:
            errors['plan'] = "Job description must be at least 3 characters"
...
class Trip(models.Model):
    ...
    trips = models.ForeignKey(User, null=True, related_name='trips', on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
